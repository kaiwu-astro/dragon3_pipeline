...
def mock_config():
    ...
    config.fixed_width_font_context = {
    ...
    config.kw_to_stellar_type_verbose = {
        i: f"{i}:Type{i}" for i in range(16)
    }
    config.st_verbose_to_color = {
        f"{i}:Type{i}": f"C{i}" for i in range(16)
    }
    config.star_type_verbose_to_marker = {
        f"{i}:Type{i}": 'o' for i in range(16)
    }
...
class TestBaseHDF5Visualizer:
    ...
    def test_decorate_jointfig(self, mock_close, mock_config, sample_dataframe):
        ...
        vis.decorate_jointfig(
            ax, sample_dataframe, 'X [pc]', 'Y [pc]',
            (-10, 10), (-10, 10), 'test_sim',
            1.0, 10.0, 5.0, 2.0
        )
...
class TestSingleStarVisualizer:
    ...
    def test_create_mass_distance_plot_density(
        self, mock_close, mock_makedirs, mock_exists, 
        mock_config, sample_dataframe, temp_dir
    ):
...
class TestLagrVisualizer:
    ...
    def test_create_lagr_plot_base(
        self, mock_close, mock_exists,
        mock_config, sample_lagr_dataframe, temp_dir
    ):
        ...
        # Should not raise an error
        try:
            vis.create_lagr_plot_base(
                sample_lagr_dataframe, 'test_sim', metric='rlagr'
            )
        except Exception as e:
            # Some plotting functions might fail in headless environment
            pass
